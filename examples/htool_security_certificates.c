...
static int get_cert_data(const struct htool_invocation* inv, uint32_t major_command,
...
int htool_get_attestation_pub_cert(const struct htool_invocation* inv) {
  ...
  return get_cert_data(inv, HOTH_PRV_CMD_HOTH_SECURITY_V2_GET_CERTIFICATES_MAJOR_COMMAND,
                    HOTH_PRV_CMD_HOTH_SECURITY_V2_GET_ATTESTATION_PUB_CERT_MINOR_COMMAND,
                    cert, ATTESTATION_CERT_SIZE);
}
...
int htool_get_device_id_cert(const struct htool_invocation* inv) {
  ...
  if (*endorsement_cert_output_file != '\0') {
    ...
    if (endorsement_cert_output_ptr == NULL) {
      printf("Error: %s, when attempting to open file: %s. Writing to STDOUT\n", strerror(errno),
