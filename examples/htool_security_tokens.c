...
static int htool_get_tokens_in_set(const struct htool_invocation* inv,
  ...
  switch (sv) {
    case LIBHOTH_SECURITY_V2: {
      ...
      printf("boot nonce written to: %s\n", boot_nonce_output_file);
...
static int htool_get_token_set_count(const struct htool_invocation* inv,
  ...
  switch (sv) {
    case LIBHOTH_SECURITY_V2: {
      ...
      printf("token count written to: %s\n", num_ids_file);
...
static int htool_get_token_set_info(const struct htool_invocation* inv,
  ...
  switch (sv) {
    case LIBHOTH_SECURITY_V2: {
      ...
      printf("token set info written to: %s\n", token_set_info_file);
      break;
    }
    // SECURITY_V3 not supported yet.
    default:
...
int htool_fetch_attestation(const struct htool_invocation* inv) {
  ...
  //Get Token Set Count
  uint32_t num_ids = 0;
  status = htool_get_token_set_count(inv, &num_ids);
  if (status != 0) {
    return status;
  }
  ...
  for (int i = 0; i < num_ids; i++) {
    //Get Tokens in Set
    status = htool_get_tokens_in_set(inv, i);
    if (status != 0) {
      return status;
    }

    //Get Token Set Info
    status = htool_get_token_set_info(inv, i);
    if (status != 0) {
      return status;
    }
  }
